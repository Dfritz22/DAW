...
static DWORD WINAPI WasapiRenderThreadProc(LPVOID param) {
    ...
    while (!audio->audioStopRequested.load()) {
        ...
        // Phase 24 / Step K5c.4 — audio callback is lock-free. Per-block
        // mix state is read from the atomic MixSnapshot; the audioStateLock
        // guard has been removed. See device_mme.cpp for the full rationale.
